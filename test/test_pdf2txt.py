...
class TestConvertPdfToText:
    ...
    def test_invalid_pdf_handling(self, temp_output_dir):
        ...
        invalid_pdf = temp_output_dir.joinpath("invalid.pdf")
    ...
    def test_empty_pdf(self, temp_output_dir):
        ...
        try:
            ...
            empty_pdf = temp_output_dir.joinpath("empty.pdf")
            writer = PdfWriter()
            writer.add_page()
    ...
    def test_multiple_pages(self, temp_output_dir):
        ...
        try:
            from PyPDF2 import PdfWriter
            multi_pdf = temp_output_dir.joinpath("multi_page.pdf")
            writer = PdfWriter()
    ...
    def test_special_characters(self, temp_output_dir):
        ...
        try:
            from PyPDF2 import PdfWriter
            special_pdf = temp_output_dir.joinpath("special_chars.pdf")
            writer = PdfWriter()
            page = writer.add_page()
...
class TestPdf2TxtMain:
    ...
    def test_main_single_file(self, sample_pdf_path, temp_output_dir, capsys):
        ...
        args = ["-i", str(sample_pdf_path), "-o", str(temp_output_dir)]
        main(args)
    ...
    def test_main_directory(self, fixtures_dir, temp_output_dir, capsys):
        ...
        # Copy sample PDF to directory
        sample_pdf = Path(fixtures_dir, "sample.pdf")
        if sample_pdf.exists():
            import shutil
            shutil.copy(sample_pdf, pdf_dir.joinpath("test1.pdf"))
            shutil.copy(sample_pdf, pdf_dir.joinpath("test2.pdf"))
        
        args = ["-i", str(pdf_dir), "-o", str(temp_output_dir)]
        main(args)
    ...
    def test_main_nonexistent_output_dir(self, sample_pdf_path, temp_output_dir):
        ...
        args = ["-i", str(sample_pdf_path), "-o", str(new_output)]
        main(args)

...
class TestPdfToKeywordsPipeline:
    ...
    def test_full_pdf_pipeline(self, sample_pdf_path, temp_output_dir):
        ...
        txt_output_dir = temp_output_dir.joinpath("txt")
    ...
    def test_multiple_pdfs_pipeline(self, fixtures_dir, temp_output_dir):
        ...
        pdf_dir = temp_output_dir.joinpath("pdfs")
        pdf_dir.mkdir()
        
        if (fixtures_dir.joinpath("sample.pdf")).exists():
            import shutil
            shutil.copy(fixtures_dir.joinpath("sample.pdf"), pdf_dir.joinpath("doc1.pdf"))
            shutil.copy(fixtures_dir.joinpath("sample.pdf"), pdf_dir.joinpath("doc2.pdf"))
        ...
        for pdf_file in pdf_files:
            convert_pdf_to_text(pdf_file, txt_output_dir)
...
class TestFullClassificationPipeline:
    ...
    def test_pdf_to_classification_pipeline(self, fixtures_dir, temp_output_dir):
        ...
        txt_dir = temp_output_dir.joinpath("txt")
        txt_dir.mkdir()
        ...
        classification_output_dir = temp_output_dir.joinpath("classification")
    ...
    def test_error_propagation(self, temp_output_dir):
        ...
        invalid_txt = temp_output_dir.joinpath("invalid.txt")
...
class TestEndToEndWorkflow:
    ...
    @pytest.mark.requires_model
    @pytest.mark.slow
    def test_complete_workflow(self, fixtures_dir, temp_output_dir):
        ...
        if (fixtures_dir.joinpath("sample.pdf")).exists():
            import shutil
            shutil.copy(fixtures_dir.joinpath("sample.pdf"), input_dir.joinpath("doc1.pdf"))
        
        if (fixtures_dir.joinpath("sample.html")).exists():
            import shutil
            shutil.copy(fixtures_dir.joinpath("sample.html"), input_dir.joinpath("doc2.html"))
        ...
        # Step 2: Extract keywords
        keyword_dir = temp_output_dir.joinpath("keywords")
        if len(list(txt_dir.glob("*.txt"))) > 0:
            ...
            if len(keyword_csvs) >= 2:
                classification_dir = temp_output_dir.joinpath("classification")
                classify_keywords_split_files(

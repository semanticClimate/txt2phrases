...
class TestClassifyKeywordsSplitFiles:
    ...
    def test_basic_classification(self, temp_output_dir):
        ...
        chapter1_data = {
            "keyword": ["machine learning", "climate change", "data science", "AI", "research"],
            "count": [15, 12, 10, 2, 1]
        }
        df1 = pd.DataFrame(chapter1_data)
        df1.to_csv(Path(input_dir, "chapter1.csv"), index=False)
        
        # Chapter 2: ML-focused keywords
        chapter2_data = {
            "keyword": ["climate change", "global warming", "carbon emissions", "sustainability", "environment"],
            "count": [20, 18, 15, 3, 12]
        }
        df2 = pd.DataFrame(chapter2_data)
        df2.to_csv(Path(input_dir, "chapter2.csv"), index=False)
        ...
        assert (Path(output_dir, "chapter1_specific_keywords.csv")).exists(), "chapter1_specific_keywords.csv should exist"
        assert (Path(output_dir, "chapter2_specific_keywords.csv")).exists(), "chapter2_specific_keywords.csv should exist"
        assert (Path(output_dir, "general_specific_keywords.csv")).exists(), "general_specific_keywords.csv should exist"
    def test_threshold_filtering(self, temp_output_dir):
        ...
        specific_df = pd.read_csv(Path(output_dir, "chapter1_specific_keywords.csv"))
    ...
    def test_missing_columns(self, temp_output_dir):
        ...
        df.to_csv(Path(input_dir, "invalid.csv"), index=False)
    ...
    def test_single_chapter(self, temp_output_dir):
        ...
        df.to_csv(Path(input_dir, "single_chapter.csv"), index=False)
        ...
        assert (Path(output_dir, "single_chapter_specific_keywords.csv")).exists(), "(Path(output_dir, single_chapter_specific_keywords.csv)) should exist"
        assert (Path(output_dir, "general_specific_keywords.csv")).exists(), "(Path(output_dir, 'general_specific_keywords.csv)) should exist"
    def test_multiple_chapters(self, temp_output_dir):
        ...
        for i in range(3):
            ...
            df = pd.DataFrame(chapter_data)
            df.to_csv(Path(input_dir, f"chapter{i+1}.csv"), index=False)
    ...
    def test_general_specific_csv_structure(self, temp_output_dir):
        ...
        general_df = pd.read_csv(Path(output_dir, "general_specific_keywords.csv"))
    ...
    def test_output_directory_creation(self, temp_output_dir):
        ...
        df = pd.DataFrame(chapter_data)
        df.to_csv(Path(input_dir, "chapter1.csv"), index=False)
        
        new_output_dir = Path(temp_output_dir, "new_output")

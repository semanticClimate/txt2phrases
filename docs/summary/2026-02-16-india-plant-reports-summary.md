...
- **Deliverables:** Per-report and merged keyword CSVs; keyphrase × institution matrix as CSV and as a jQuery DataTables HTML page.
...
- **Outputs:**  
  ...
  - **(b) HTML:** `keyphrase_datatables.html` – same data in a single file with embedded JSON and jQuery DataTables (search, sort, pagination, default sort by Total descending).
- **Run:** `python scripts/build_keyphrase_table.py` (default reports-dir: `Examples/otvirare/india_plant_reports`). Optional `--csv` and `--html` to set output paths.
...
| `keyphrase_datatables.html` | DataTables page for the matrix (open in browser; uses jQuery and DataTables from CDN). |
